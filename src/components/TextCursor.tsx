...
const TextCursor: FC<TextCursorProps> = ({
  text = 'GOA',
  spacing = 80,
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    const currentRandomFloat = randomFloatRef.current;
    const currentFollowMouse = followMouseDirectionRef.current;
    const currentMaxPoints = maxPointsRef.current;
    ...
    setTrail(prev => {
      ...
      if (newTrail.length === 0) {
        ...
        if (distance >= currentSpacing) {
          ...
          const computedAngle = currentFollowMouse ? rawAngle : 0;
          const steps = Math.floor(distance / currentSpacing);

          for (let i = 1; i <= steps; i++) {
            const t = (currentSpacing * i) / distance;
      ...
      return newTrail.length > currentMaxPoints ? newTrail.slice(newTrail.length - currentMaxPoints) : newTrail;
    });
